...
def CATEGORIES():
    addDir('Interpreten','http://musicmp3.ru/artists.html',21,art + 'artists.jpg','')
    addDir('Top Alben','http://musicmp3.ru/genres.html',12,art + 'topalbums.jpg','')
    addDir('Neue Alben','http://musicmp3.ru/new_albums.html',12,art + 'newalbums.jpg','')
    addDir('Zusammenstellungen','url',400,'','')
    addDir('Charts auf Billboard','url',101,art + 'billboardcharts.jpg','')
    addDir('Suche: Interpret','url',24,art + 'searchartists.jpg','')
    addDir('Suche: Album','url',24,art + 'searchalbums.jpg','')
    addDir('Suche: Titel','url',24,art + 'searchsongs.jpg','')
    addDir('Favorit (Interpret)','url',63,art + 'favouriteartists.jpg','')
    addDir('Favorit (Album)','url',66,art + 'favouritealbums.jpg','')
    addDir('Favorit (Titel)','url',69,art + 'favouritesongs.jpg','')
    addDirAudio('Zuf. Wiedergabe Favorisierter Titel (Sortieren und Abspielen)','url',99,art + 'mixfavouritesongs.jpg','','','','','')
    addDirAudio('Zuf. Wiedergabe Favorisierter Alben (Sortieren und Abspielen)','url',89,art + 'mixfavouritealbums.jpg','','','','','')
    addDirAudio('Playliste leeren','url',100,art + 'clearplaylist.jpg','','','','','')
    addDirAudio('ID3 Tags Eintragen','url',300,art + 'addid3tags.jpg','','','','','')
    addDir('Durchsuche alternative Quellen','url',700,artgenre + 'alternate.jpg','')
...
def artists(url):
    link = GET_url(url)
    addDir('Alle Interpreten','http://musicmp3.ru/main_artists.html?type=artist&page=1',31,art + 'allartists.jpg','')
...
def all_artists(name, url):
    ...
    addDir('>> Weiter',nxtpgurl,31,xbmc.translatePath(os.path.join('special://home/addons/plugin.audio.mp3streams', 'art', 'nextpage.jpg')),'')
...
def all_genres(name, url):
    ...
    addDir('>> Weiter',nxtpgurl,13,xbmc.translatePath(os.path.join('special://home/addons/plugin.audio.mp3streams', 'art', 'nextpage.jpg')))
...
def compilations_list(name, url, iconimage, page):
    ...
    addDir('>> Weiter',nxtpgurl,401,xbmc.translatePath(os.path.join('special://home/addons/plugin.audio.mp3streams', 'art', 'nextpage.jpg')),str(nextpage))
...
def album_list(name, url):
    ...
    nxtpgurl = url[:pgnumf]
    nxtpgurl = "%s%s" % (nxtpgurl, pgnum)
    addDir('>> Weiter',nxtpgurl,15,xbmc.translatePath(os.path.join('special://home/addons/plugin.audio.mp3streams', 'art', 'nextpage.jpg')),'')
    setView('movies', 'album')
...
def add_favourite(name, url, dir, text):
    ...
    if 'artist' in dir:
        ...
        notification(name.upper(), "[COLOR blue]" + text + "[/COLOR]", '5000','')
        link = GET_url(url1)
        try:
            artist_url = regex_from_to(link, 'class="art_wrap__img" src="', '"')
        ...
        notification(name.upper(), "[COLOR blue]" + text + "[/COLOR]", '5000', thumb)
...
def add_favourite_song(name, url, dir, text):
    ...
    url = "%s<>%s" % (url, plname)
    add_to_list(url, dir, False)
    notification(songname.upper(), "[COLOR blue]" + text + "[/COLOR]", '5000',iconimage)
...
def addDir(name, url, mode, iconimage, type):
        ...
        if type == "artists":
            if find_list(list, FAV_ARTIST) < 0:
                suffix = ""
                contextMenuItems.append(("[COLOR blue]Add to Favourite Artists[/COLOR]",'XBMC.RunPlugin(%s?name=%s&url=%s&mode=61)'%(sys.argv[0], name, str(list))))
            else:
                suffix = ' [COLOR blue]+[/COLOR]'
        ...
        if 'album' in type:
            ...
            if not 'qq' in type1:
                suffix = ""
                contextMenuItems.append(("[COLOR blue]Add to Favourite Albums[/COLOR]",'XBMC.RunPlugin(%s?name=%s&url=%s&mode=64)'%(sys.argv[0], name.replace(',', ''), str(list))))
            else:
                suffix = ' [COLOR blue]+[/COLOR]'
...
def addDirAudio(name, url, mode, iconimage, songname, artist, album, dur, type):
        ...
        if type != 'favsong':
            suffix = ""
            contextMenuItems.append(("[COLOR blue]Add to Favourite Songs[/COLOR]",'XBMC.RunPlugin(%s?name=%s&url=%s&mode=67)'%(sys.argv[0], name.replace(',', ''), str(list))))
        else:
            suffix = ' [COLOR blue]+[/COLOR]'
